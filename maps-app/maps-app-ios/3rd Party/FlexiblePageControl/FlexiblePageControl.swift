...
public class FlexiblePageControl: UIView {
    ...
    public var pageIndicatorTintColor = UIColor(red: 0.86, green: 0.86, blue: 0.86, alpha: 1.00) {
        didSet {
            updateDotColor(currentPage: currentPage)
        }
    }

    public var currentPageIndicatorTintColor = UIColor(red: 0.32, green: 0.59, blue: 0.91, alpha: 1.00) {
        didSet {
            updateDotColor(currentPage: currentPage)
        }
    }
...
private class ItemView: UIView {
    ...
    private func updateDotSize(state: State) {
        ...
        case .medium:
            size = CGSize(width: dotSize * ItemView.mediumSizeRatio, height: dotSize * ItemView.mediumSizeRatio)
        case .small:
            size = CGSize(width: dotSize * ItemView.smallSizeRatio, height: dotSize * ItemView.smallSizeRatio)
        case .none:
            size = CGSize.zero
        }

        dotView.layer.cornerRadius = size.height / 2.0
